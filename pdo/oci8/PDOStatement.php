...
class PDOStatement extends \PDOStatement
{
     ...
     * @return void
     */
    public function __construct($sth, PDO $pdoOci8, array $options = array())
    {
    ...
    public function bindParam($parameter, &$variable,
                              $data_type = PDO::PARAM_STR, $length = -1,
                              $driver_options = null)
    {
        ...
        if ($data_type === \PDO::PARAM_LOB) {
            if (!is_object($variable)) {
                $connection = $this->_pdoOci8->getConnection();
                $descriptor = oci_new_descriptor($connection, OCI_D_LOB);
                $descriptor->writetemporary($variable, OCI_TEMP_BLOB);
            } else {
                $descriptor = $variable;
            }
            return oci_bind_by_name($this->_sth, $parameter, $descriptor, -1,
                    OCI_B_BLOB);
        } elseif (is_array($variable)) {
            return oci_bind_array_by_name(
                    $this->_sth, $parameter, $variable, count($variable),
                    $length
            );
        } else {
            if ($length == -1) {
    ...
    public function fetchAll($fetch_style = PDO::FETCH_BOTH,
                             $fetch_argument = null, $ctor_args = null)
    {
        if ($this->_fetchMode !== null) {
            $fetch_style = $this->_fetchMode;
        }
        ...
        if ($fetch_style === PDO::FETCH_ASSOC) {
            ...
            oci_fetch_all($this->_sth, $preResult, 0, -1,
     ...
     * @return bool
     */
    public function setFetchMode($mode, $colClassOrObj = null,
